...
TEMPLATES = {14: "Хост шаблон-1", 13: "Хост шаблон-2", 11: "МД шаблон-1", 5: "МД шаблон-2", 7: "СИ шаблон-1"}

def safe(value): return os.path.basename(value or "")
def project_dir(project): return os.path.join(PROJECTS_ROOT, safe(project)) if project else None
def single_project(project): return bool(project and os.path.isdir(os.path.join(project_dir(project), "playbooks")))
def get_projects():
...
def get_objects(project):
    if single_project(project): return []
...
def object_dir(project, obj):
    if single_project(project): return os.path.join(project_dir(project), "playbooks")
...
def read(path):
    try:
        with open(path, encoding="utf-8") as file: return file.read()
...
def load_inventory(project, obj):
    ...
    if not file_paths or not os.path.exists(file_paths["hosts"]): return {}
    try: return yaml.safe_load(read(file_paths["hosts"])) or {}
    except yaml.YAMLError as error:
        log(f"HOSTS YAML ERROR: {error}")
        return {}
def load_hosts(project, obj): return load_inventory(project, obj).get("all", {}).get("hosts", {}) or {}
def classify_node(params):
    ...
    if last_key == "uefi": return "host", "Хост"
    if last_key == "Description": return "md", "МД"
    return "unknown", "Узел"
...
def template_schemas(project, obj):
    ...
    for node in parse_hosts(project, obj):
        ...
        if template not in ("Хост", "МД", "Узел") and template not in schemas: schemas[template] = list(node["parameters"])
    return schemas
def inventory_groups(project, obj):
    data = load_inventory(project, obj); wanted = {"arms", "servers", "md"}; groups = {}
    def collect(value):
        found = set()
        if isinstance(value, dict):
            hosts = value.get("hosts", {})
            if isinstance(hosts, dict): found.update(hosts.keys())
            for child in (value.get("children", {}) or {}).values(): found.update(collect(child))
        return found
    for name, value in data.items():
        if name in wanted: groups[name] = sorted(collect(value))
...
def add_host(project, obj, name, values):
    file_paths = paths(project, obj); data = load_inventory(project, obj); hosts = data.setdefault("all", {}).setdefault("hosts", {})
    if not name: raise ValueError("Имя узла не указано")
    if name in hosts: raise ValueError("Узел уже существует")
    normalized_values = {key: scalar(value) for key, value in values.items()}; hosts[name] = normalized_values
    raw = read(file_paths["hosts"]); lines = raw.splitlines(keepends=True); hosts_index, end_index = _hosts_section_bounds(lines)
    if hosts_index is None:
        save_hosts(file_paths["hosts"], data); return
    newline = "\r\n" if "\r\n" in raw else "\n"; entry_indexes = _host_entry_indexes(lines, hosts_index, end_index)
    if entry_indexes:
        insert_at = end_index
        while insert_at > hosts_index + 1 and not lines[insert_at - 1].strip(): insert_at -= 1
        prefix = newline if lines[insert_at - 1].strip() else ""
    else:
        insert_at = hosts_index + 1
        while insert_at < end_index and not lines[insert_at].strip(): insert_at += 1
        prefix = ""
    block = _host_yaml_block(name, normalized_values, newline)
    if entry_indexes: block = prefix + block
    if end_index < len(lines): block += newline
    lines[insert_at:insert_at] = [block]
    write(file_paths["hosts"], "".join(lines))

def delete_host(project, obj, name):
    file_paths = paths(project, obj); raw = read(file_paths["hosts"]); lines = raw.splitlines(keepends=True); hosts_index, end_index = _hosts_section_bounds(lines)
    if hosts_index is None: raise ValueError("Узел не найден")
    entry_indexes = _host_entry_indexes(lines, hosts_index, end_index); target_index = None
    for index in entry_indexes:
        if _host_name_from_line(lines[index]) == name: target_index = index; break
    if target_index is None: raise ValueError("Узел не найден")
...
def save_host(project, obj, old_name, new_name, values):
    file_paths = paths(project, obj); data = load_inventory(project, obj); hosts = data.setdefault("all", {}).setdefault("hosts", {})
    if old_name not in hosts: raise ValueError("Узел не найден")
    if new_name != old_name and new_name in hosts: raise ValueError("Узел с таким именем уже существует")
    updated = dict(hosts[old_name]) if isinstance(hosts[old_name], dict) else {}
    for key, value in values.items(): updated[key] = scalar(value)
    hosts.pop(old_name); hosts[new_name] = updated; save_hosts(file_paths["hosts"], data)
...
def playbook_roles(project, obj, playbook_name):
    ...
    if not os.path.isfile(playbook_path): return []
    try: document = yaml.safe_load(read(playbook_path)) or []
    except yaml.YAMLError as error:
        log(f"PLAYBOOK YAML ERROR: {error}"); return []
    names = []
    def inspect(value):
    ...
    inspect(document); root = roles_dir(project, obj)
    if not root or not os.path.isdir(root): return []
    result = []; seen = set(); root_abs = os.path.abspath(root)
...
def role_file(project, obj, relative_path):
    ...
    root = os.path.abspath(root); path = os.path.abspath(os.path.join(root, relative_path))
    ...
    while True:
        ...
        for project in get_projects():
            for obj in (get_objects(project) or [None]):
                key = obj or ""; statuses.setdefault(project, {})[key] = {node["hostname"]: host_up(node["ip"]) for node in parse_hosts(project, obj)}
        with STATUS_LOCK: HOST_STATUS = statuses
        time.sleep(10)
def status(project, obj):
    with STATUS_LOCK: return dict(HOST_STATUS.get(project, {}).get(obj or "", {}))
def run_playbook(command, project, obj, name, cwd, cfg, label=None):
    title = label or name; log(f"=== START {title} [{project}{('/' + obj) if obj else ''}] ==="); process = None
    try:
    ...
    except OSError as error: log(f"EXECUTION ERROR: {error}")
    finally:
        if process:
            with PROC_LOCK:
                if process in PROCESSES: PROCESSES.remove(process)
def run_command(project, obj, names, hosts):
    file_paths = paths(project, obj); available = {item["name"]: item["path"] for item in get_playbooks(project, obj)}
    if not file_paths: return
    for name in names:
...
def run_autodeploy(project, hosts):
    ...
    if not path: return
    file_paths = paths(project, None); command = ["ansible-playbook", path]
    if hosts: command += ["-l", ",".join(hosts)]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        url = urlparse(self.path); query = parse_qs(url.query); project = query.get("project", [""])[0]; obj = query.get("object", [""])[0]
        ...
        if url.path in static:
            filename = static[url.path]; content_type = "text/html; charset=utf-8" if filename.endswith(".html") else "text/css; charset=utf-8" if filename.endswith(".css") else "application/javascript; charset=utf-8"; self.file(os.path.join(PUBLIC_DIR, filename), content_type); return
        ...
        if url.path == "/playbook":
            name = safe(query.get("name", [""])[0]); file_paths = paths(project, obj); self.json({"name": name, "content": read(os.path.join(file_paths["object_dir"], name)) if file_paths else ""}); return
        if url.path == "/files":
            file_paths = paths(project, obj); self.json({"hosts": read(file_paths["hosts"]) if file_paths else "", "defaults": read(file_paths["defaults"]) if file_paths else ""}); return
        self.send_error(404)
    def do_POST(self):
        try:
        ...
        try:
            if self.path == "/run": threading.Thread(target=run_command, args=(project, obj, data.get("playbooks", []), data.get("hosts", [])), daemon=True).start(); self.json({"ok": True}); return
            if self.path == "/run_autodeploy": threading.Thread(target=run_autodeploy, args=(project, data.get("hosts", [])), daemon=True).start(); self.json({"ok": True}); return
            if self.path == "/stop": stop(); self.json({"ok": True}); return
            if self.path == "/update_host": save_host(project, obj, data.get("hostname", ""), data.get("new_hostname", data.get("hostname", "")), data.get("values", {})); self.json({"ok": True}); return
            if self.path == "/add_host": add_host(project, obj, data.get("hostname", ""), data.get("values", {})); self.json({"ok": True}); return
            if self.path == "/delete_host": delete_host(project, obj, data.get("hostname", "")); self.json({"ok": True}); return
            ...
            if self.path == "/save_files":
                ...
                self.json({"ok": True}); return
        except (OSError, ValueError) as error: self.json({"ok": False, "error": str(error)}, 500); return
        self.send_error(404)

if __name__ == "__main__":
    threading.Thread(target=status_worker, daemon=True).start(); HTTPServer(("0.0.0.0", 8000), Handler).serve_forever()
